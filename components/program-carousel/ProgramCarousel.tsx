...
import { ArrowRight, ChevronLeft, ChevronRight, X, CheckCircle2 } from "lucide-react";
...
export default function ProgramCarousel() {
  ...
  return (
    <section id="programs" className="py-24 bg-cream-light border-y border-darkteal/15 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-2.5">
            <button
              onClick={() => scroll("left")}
              className="w-10 h-10 rounded-lg bg-cream hover:bg-darkteal hover:text-cream text-softblack border border-darkteal/20 flex items-center justify-center transition-all duration-200 shadow-sm"
        ...
        >
          {PROGRAMS_DATA.map((program, index) => (
            ...
            >
              <TiltCard onClick={() => handleOpenProgram(program)} className="h-full">
                <div className="editorial-card h-full rounded-2xl overflow-hidden bg-cream cursor-pointer group flex flex-col justify-between border border-darkteal/20 hover:border-darkteal/50">
                  <div>
                    {/* Image */}
                    <div className="relative h-48 w-full overflow-hidden">
                      ...
                      <div className="absolute top-4 left-4">
                        <span className="inline-block px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-mint text-softblack shadow-md border border-darkteal/15">
                          {program.category}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="px-6 pb-6 pt-2 flex items-center justify-between border-t border-darkteal/10 mt-auto">
                    ...
                    <div className="w-8 h-8 rounded-md bg-mint-light group-hover:bg-darkteal group-hover:text-cream text-darkteal flex items-center justify-center transition-all duration-200 shadow-sm">
            ...
            >
              ...
              <div className="relative h-56 sm:h-64 w-full">
                ...
                <div className="absolute bottom-4 left-4">
                  <span className="inline-block px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider bg-mint text-softblack shadow-md">
              ...
              <div className="p-6 sm:p-8 overflow-y-auto space-y-6">
                ...
                <div className="p-5 rounded-xl bg-mint-light/40 border border-darkteal/20 flex items-start gap-3">
              ...
              <div className="p-4 sm:p-6 bg-cream-light border-t border-darkteal/10 flex justify-end">
                <button
                  onClick={() => setSelectedProgram(null)}
                  className="px-5 py-2 rounded-lg bg-darkteal text-cream text-xs font-bold uppercase tracking-wider hover:bg-teal transition-colors shadow-sm"
                >
                  Tutup Informasi
                </button>
              </div>
